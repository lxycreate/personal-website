...
#  Timestep    No_Moles    No_Specs C136H115O38          O2
       1000         105           2           5         100
#  Timestep    No_Moles    No_Specs C136H115O38          O2`)],-1),L=w({__name:"format-first",setup(F){const m=b([]),x=(e,t)=>{const c=_.json_to_sheet(e),l=_.book_new();_.book_append_sheet(l,c,"Sheet1"),M(l,`${t}.csv`,{bookType:"csv",type:"binary"})},S=e=>{const t=new FileReader;t.onload=c=>{const C=c.target.result.split(`
`),u=[];C.forEach((s,o)=>{const n=s.split(/\s+/);u.push(n.filter(i=>!["","#"].includes(i)))});let p=new Set,N=[],O=[];const r=[];u.forEach((s,o)=>{if(o%2==0)N=s,p=new Set([...p,...s]);else{O=s;const n={};N.forEach((i,H)=>{n[i]=O[H]}),r.push(n)}}),r.forEach(s=>{p.forEach(o=>{s[o]=s[o]||""})}),x(r,e.name)},t.readAsText(e)},T=e=>(m.value=[e],S(e),!1);return(e,t)=>(y(),v("div",V,[d(f(U),{"file-list":m.value,"before-upload":T},{default:h(()=>[d(f(B),{type:"primary"},{default:h(()=>[a("div",g,[d(f(k),{class:"mr-[4px]"}),E("点击上传 ")])]),_:1})]),_:1},8,["file-list"]),j]))}});export{L as default};
