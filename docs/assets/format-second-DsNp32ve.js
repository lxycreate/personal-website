...
`),r=[];b.forEach((c,y)=>{const s=c.split(/\s+/);r.push(s.filter(l=>l))});const w=r.shift(),m=[];r.forEach((c,y)=>{const s={};w.forEach((l,E)=>{s[l]=c[E]||""}),m.push(s)}),_(m,e.name)},t.readAsText(e)},h=e=>(p.value=[e],x(e),!1);return(e,t)=>(F(),U("div",S,[d(f(T),{"file-list":p.value,"before-upload":h},{default:u(()=>[d(f(B),{type:"primary"},{default:u(()=>[o("div",V,[d(f(k),{class:"mr-[4px]"}),N("点击上传 ")])]),_:1})]),_:1},8,["file-list"]),C]))}});export{P as default};
